...
namespace TestForm
{
    ...
    {
        private EventSchedule projectManager = new EventSchedule();
        public Form1()
        {
            InitializeComponent();
            schedulerControl1.Init(projectManager);
            projectManager.Start = DateTime.Now.AddDays(-3);
            var task = new ScheduleEvent(1) { Name = "Hello" , Slack = TimeSpan.FromDays(0.5)};
            ...
            var task2 = new ScheduleEvent(0) { Name = "Hello" };
            ...
            var task3 = new ScheduleEvent(0) { Name = "Hello" };
